...
def call_llm(question: str, state: dict, fallback_facts: str) -> str | None:
    ...
    if provider == "gemini":
        return _call_gemini(api_key, system_prompt, user_prompt)
    return None


def _call_gemini(api_key: str, system_prompt: str, user_prompt: str) -> str | None:
    """
    Uses Gemini's plain generateContent REST endpoint (no function/tool
    calling) — the advisor only needs the model to phrase an answer from
    facts we already computed, so automatic function calling is unnecessary
