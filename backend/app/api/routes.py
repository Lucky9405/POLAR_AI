...
def energy_history(limit: int = 200):
    rt = _runtime()
    return {"history": DB.get_recent_telemetry(rt.code, limit=limit)}
...
def carbon_analytics():
    rt = _runtime()
    history = DB.get_recent_telemetry(rt.code, limit=5000)
    return asdict(compute_carbon_summary(history))
...
def digital_twin():
    ...
    return {
        "station": rt.code,
        "nodes": {
            "solar": {"output_kw": tick["solar_kw"], "capacity_kw": None, "active": tick["solar_kw"] > 0.5},
            "wind": {"output_kw": tick["wind_kw"], "active": tick["wind_kw"] > 0.5},
            "battery": {"soc_pct": tick["battery_soc_pct"], "power_kw": tick["battery_power_kw"],
                        "charging": tick["battery_power_kw"] > 0},
            "diesel": {"on": tick["diesel_on"], "output_kw": tick["diesel_output_kw"],
                       "fuel_liters": tick["diesel_fuel_liters"]},
            "load": {"critical_kw": tick["load_critical_kw"], "important_kw": tick["load_important_kw"],
                     "flexible_kw": tick["load_flexible_kw"], "curtailed_kw": tick["flexible_curtailed_kw"]},
        },
        "flows": {
            "solar_to_bus": tick["solar_kw"],
            "wind_to_bus": tick["wind_kw"],
            "battery_to_bus": max(0, -tick["battery_power_kw"]),
            "bus_to_battery": max(0, tick["battery_power_kw"]),
            "diesel_to_bus": tick["diesel_output_kw"],
        },
        "mode": rt.mode,
    }
