...
class StationConfig(BaseModel):
    # ---- Load profile (kW) ----
    # Four-tier priority: CRITICAL > ESSENTIAL > FLEXIBLE > DEFERRABLE.
    base_load_kw: float = 60.0          # always-on baseline
    critical_load_kw: float = 40.0      # safety/emergency/life-support ceiling
    essential_load_kw: float = 45.0     # comms/research-infra/core-ops ceiling
    flexible_load_kw: float = 35.0      # workshop/non-critical operational ceiling
    deferrable_load_kw: float = 15.0    # optional/non-urgent ceiling
    ...
    battery_min_soc_pct: float = 15.0        # hard floor — never discharge below this
    battery_normal_reserve_pct: float = 35.0    # standard operating reserve target (configurable, single source of truth)
    battery_survival_reserve_pct: float = 40.0  # elevated reserve target in survival mode
    ...
    initial_fuel_liters: float = 2600.0
    fuel_tank_capacity_liters: float = 3000.0
