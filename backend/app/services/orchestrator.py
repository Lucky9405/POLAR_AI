...
def _decide_for(last: dict, renewable_drop: float, survival_override: bool | None = None, previous_mode: str = "NORMAL"):
    return optimizer_decide(
        battery_soc_pct=last["battery_soc_pct"],
        renewable_kw=last["solar_kw"] + last["wind_kw"],
        load_critical_kw=last["load_critical_kw"],
        load_essential_kw=last["load_essential_kw"],
        load_flexible_kw=last["load_flexible_kw"],
        load_deferrable_kw=last["load_deferrable_kw"],
...
def advance_tick(runtime, survival_override: bool | None = None) -> dict:
    ...
    if last:
        ...
        decision = _decide_for(last, renewable_drop, survival_override, previous_mode=runtime.mode)
    else:
        decision = optimizer_decide(70, 50, 40, 45, 35, 15, CFG.initial_fuel_liters, 10, 0, survival_override)
    ...
    runtime.mode = decision.operating_mode
    ...
    risk = compute_risk_score(
        ...
        predicted_deficit_kw=tick_dict["flexible_curtailed_kw"] + tick_dict["deferrable_curtailed_kw"],
...
def get_state_snapshot(runtime) -> dict:
    ...
    last = history[-1]
    renewable_drop = _forecast_renewable_drop_pct(history)
    decision = _decide_for(last, renewable_drop, previous_mode=runtime.mode)
    autonomy = compute_fuel_autonomy(
    ...
    risk = compute_risk_score(
        ...
        storm_probability_pct=last["weather"]["storm_probability_pct"],
        predicted_deficit_kw=last["flexible_curtailed_kw"] + last["deferrable_curtailed_kw"],
