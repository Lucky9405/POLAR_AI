...
class StationSimulator:
    ...
    def __init__(self, start_time: datetime | None = None, seed: int | None = None,
                 speed_multiplier: float = 1.0, solar_capacity_kw: float | None = None,
                 wind_capacity_kw: float | None = None, temp_offset_c: float = 0.0):
        ...
        # Station-specific profile overrides (e.g. Maitri vs Bharati) — default
        # to the shared config values when not provided, so a plain
        # StationSimulator() behaves exactly as before.
        self.solar_capacity_kw = solar_capacity_kw if solar_capacity_kw is not None else CFG.solar_capacity_kw
        self.wind_capacity_kw = wind_capacity_kw if wind_capacity_kw is not None else CFG.wind_capacity_kw
    ...
    def _weather_at(self, t: datetime) -> WeatherState:
        ...
        seasonal = -15 + 8 * math.sin(2 * math.pi * (day_of_year / 365.0)) + self.temp_offset_c
    ...
    def _solar_kw(self, weather: WeatherState) -> float:
        ...
        raw = self.solar_capacity_kw * (weather.solar_irradiance_wm2 / 1000.0)
    ...
    def _load_kw(self, t: datetime, weather: WeatherState):
        ...
        essential = CFG.essential_load_kw * activity * cold_factor * (0.9 + 0.1 * self.rng.uniform(0, 1))
        flexible = CFG.flexible_load_kw * activity * (0.8 + 0.2 * self.rng.uniform(0, 1))
        deferrable = CFG.deferrable_load_kw * activity * (0.7 + 0.3 * self.rng.uniform(0, 1))

        critical = min(critical, CFG.critical_load_kw)
        essential = min(essential, CFG.essential_load_kw)
        flexible = min(flexible, CFG.flexible_load_kw)
        deferrable = min(deferrable, CFG.deferrable_load_kw)

        return round(critical, 2), round(essential, 2), round(flexible, 2), round(deferrable, 2)
    ...
    def step(self, battery_command_kw: float | None = None,
        ...
        critical, essential, flexible, deferrable = self._load_kw(t, weather)
        load_total = critical + essential + flexible + deferrable
        ...
        # If still short after battery+diesel, curtail load in strict priority
        # order: DEFERRABLE first, then FLEXIBLE. CRITICAL and ESSENTIAL are
        # never curtailed by the physical model (the optimizer is expected to
        # have committed enough diesel/battery to protect them; this is a
        # last-resort physical clamp, not the primary protection mechanism).
        still_short = remaining_deficit - diesel_output
        if still_short > 0.1:
            deferrable_curtailed = min(deferrable, still_short)
            deferrable -= deferrable_curtailed
            still_short -= deferrable_curtailed
            load_total -= deferrable_curtailed
        if still_short > 0.1:
        ...
        weather_dict["timestamp"] = t.isoformat()  # datetime -> str (JSON-serializable; avoids a latent DB bug)
        ...
        tick = StationTick(
            ...
            load_essential_kw=round(essential, 2),
            load_flexible_kw=round(flexible, 2),
            load_deferrable_kw=round(deferrable, 2),
            flexible_curtailed_kw=round(flexible_curtailed, 2),
            deferrable_curtailed_kw=round(deferrable_curtailed, 2),
            solar_kw=solar_kw,
            wind_kw=wind_kw,
